...
def _get_data_dir():
    """数据目录：exe 同级的 Hook_data 文件夹，方便迁移到任意盘"""
    ...
    try:
        ...
        # 降级：APPDATA
        d = Path(os.environ.get('APPDATA', Path.home())) / 'Hook'
        d.mkdir(parents=True, exist_ok=True)
    return d
...
# 首次运行标记放在用户目录，移动 exe 也不会重复显示引导
FIRST_RUN     = Path.home() / '.hook_first_run_done'
...
def 写历史(r):
    ...
    HISTORY_FILE.write_text(json.dumps(h[:200], ensure_ascii=False, indent=2), 'utf-8')
...
def safe_get(session, url, headers, timeout=15, **kw):
    try:
...
# 广告过滤模式
_AD_PATTERNS = [
    re.compile(r'seb\s*ook[\s\S]{0,20}com', re.I),
    re.compile(r'www\.\S+\.(com|net|org|cc|xyz)', re.I),
    re.compile(r'本[书章文]来自\S+', re.I),
    re.compile(r'请到\S+阅读', re.I),
    re.compile(r'最新章节请到\S+', re.I),
    re.compile(r'[（(][^）)]{0,30}(?:书屋|小说|阅读|书城|网站)[^）)]{0,10}[）)]'),
    re.compile(r'——+来自\S+'),
    re.compile(r'〔[^〕]{0,30}〕'),
]
...
def smart_resolve_url(url, session):
    ...
    adp = get_adapter(url)
    try:
        r = safe_get(session, url, adp.headers(url), timeout=15)
        r.encoding = r.apparent_encoding if r.encoding in ('ISO-8859-1', None) else r.encoding
        soup = BeautifulSoup(r.text, 'html.parser')
...
class DownloadThread(QThread):
    ...
    done     = pyqtSignal(int, list, list, str, str)  # count,epub,failed,path,reason
    def __init__(self, target, url, title, path, settings, session):
        ...
        self.content_cache = {}  # 暴露给外部，供终止时写入
        self.epub_data     = []  # 暴露给外部，供 epub 生成
    def stop(self):   self._stop=True
    def pause(self):  self._paused_flag=True
    def resume(self): self._paused_flag=False; self._stop=False
    def run(self):
        adp       = get_adapter(self.url)
        prog      = 读进度(self.path)
        done_urls = set(prog.get('done', []))
        failed=[]; count=0
        epub_data = self.epub_data  # 使用实例变量
        ...
        content_cache = self.content_cache  # 使用实例变量，外部可访问
        try:
            ...
            with ThreadPoolExecutor(max_workers=CONCURRENCY) as ex:
                ...
                for fut in as_completed(futs):
                    if self._stop:
                        ...
                        # 暂停时写文件，并同步更新进度文件 done 列表
                        try:
                            with open(self.path, 'a', encoding='utf-8') as _f:
                                for _i in sorted(content_cache.keys()):
                                    _t, _c = content_cache[_i]
                                    if self.settings.get('gen_txt', True):
                                        _f.write(f"\n{'─'*32}\n{_t}\n{'─'*32}\n{_c}\n")
                            # 关键：把 content_cache 里的 URL 也加入 done
                            for _i, (_t, _c) in content_cache.items():
                                _url = self.target[_i][0] if _i < len(self.target) else ''
                                if _url: done_urls.add(_url)
                            prog['done'] = list(done_urls)
                            写进度(self.path, prog)
                        except: pass
                        self.log.emit(f'已暂停，当前已保存 {len(content_cache)} 章。', 'yellow')
                        break
        ...
        reason = 'paused' if self._paused else 'done'
        self.done.emit(count, epub_data, failed, self.path, reason)
...
class _ClickLineEdit(QLineEdit):
    """点击或获取焦点时弹出补全；选中后自动填入并触发识别"""
    def __init__(self, *a, **kw):
        super().__init__(*a, **kw)
        self._just_activated = False  # 防止选中后立刻重弹
    ...
    def focusInEvent(self, e):
        ...
        QTimer.singleShot(80, self._show_completer)
...
QTextEdit { background:#111; border:1px solid #222; border-radius:7px;
    color:#bbb; font-family:"Microsoft YaHei UI","Segoe UI";
...
def sec_label(t):
    l = QLabel(t)
    l.setStyleSheet('color:#666;font-size:11px;letter-spacing:0.5px;')
    return l
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self._dl_state   = 'idle'  # idle/running/paused/terminated
        self._failed     = []
        ...
        self._build()
        # 启动后置顶获取焦点
        QTimer.singleShot(100, lambda: (self.activateWindow(), self.raise_()))
    ...
    def _tab_dl(self):
        ...
        stats.setStyleSheet('color:#4a4a4a;font-size:11px;')
        ...
        self.prog_lbl.setStyleSheet('color:#888;font-size:11px;')
        self.eta_lbl = QLabel('')
        ...
        self.stop_btn.clicked.connect(self._pause_or_resume)
        ...
        self.retry_btn.setVisible(False)   # 平时隐藏，失败时才显示
        self.save_btn = QPushButton('💾  保存')
        self.save_btn.setObjectName('flat'); self.save_btn.setFixedHeight(36)
        self.save_btn.clicked.connect(self._save_and_terminate)
        self.save_btn.setVisible(False)
        self.discard_btn = QPushButton('🗑  舍弃')
        self.discard_btn.setObjectName('danger'); self.discard_btn.setFixedHeight(36)
        self.discard_btn.clicked.connect(self._discard_and_terminate)
        ...
        for b in [self.start_btn,self.sel_btn,self.stop_btn,
                  self.save_btn,self.discard_btn,self.retry_btn,self.open_btn]:
            bf.addWidget(b)
        L.addLayout(bf)
        return pg
    ...
    def _start(self):
        if self._dl_state == 'running': return  # 统一用状态机，不用 isRunning
    ...
    def _start_with(self, target):
        if self._dl_state == 'running': return  # 下载中不允许重复启动
        ...
        self._start_thread(target, save_path)

    def _on_prog(self, cur, total, eta, speed=0):
        offset = getattr(self, '_dl_done_offset', 0)
        self.prog.setValue(cur + offset)
    ...
    def _on_done(self, count, epub_data, failed, save_path, reason='done'):
        """
        reason = 'paused'  → 用户点了暂停，保持界面冻结，等待继续或终止
        reason = 'done'    → 正常完成或终止，执行完整收尾
        """
        if reason == 'paused':
            # 暂停：界面已在 _pause_or_resume 里更新，这里只打日志
            self._log('已暂停，点「继续」恢复，点「保存」或「舍弃」结束。', 'yellow')
            # 确保按钮状态正确
            self.stop_btn.setText('▶  继续')
            self.stop_btn.setStyleSheet(
                'background:#14532d;color:#4ade80;border:1px solid #166534;'
                'border-radius:7px;padding:8px 18px;font-size:13px;min-height:32px;')
            self.save_btn.setVisible(True); self.discard_btn.setVisible(True)
        ...
        self.stop_btn.setStyleSheet('')
        self.save_btn.setVisible(False); self.discard_btn.setVisible(False)
        self.retry_btn.setVisible(False)  # 先隐藏，有失败时再显示
        if failed:
        ...
        if self.settings.get('auto_open') and reason == 'done':
            d = str(Path(save_path).parent)
            if Path(d).exists(): os.startfile(d)
        # 确保 stop_btn 回到初始状态（防止下完后按钮还显示「继续」）
        self.stop_btn.setText('⏸  暂停')
        self.stop_btn.setStyleSheet('')
        self._dl_state = 'idle'
        self.book_lbl.setStyleSheet('color:#3b82f6;font-size:13px;font-weight:600;')
    ...
    def _retry(self):
        ...
        self._start_thread(self._failed, self._current_save_path)


    def _start_thread(self, target, save_path):
        """启动下载线程（供首次启动、继续下载、重试复用）"""
        # 断开旧线程信号（不 wait，避免阻塞主线程）
        if self._dl_thread is not None:
            try:
                self._dl_thread.log.disconnect()
                self._dl_thread.progress.disconnect()
                self._dl_thread.done.disconnect()
            except: pass
            if self._dl_thread.isRunning():
                self._dl_thread.stop()
            self._dl_thread.deleteLater()  # Qt安全释放
            self._dl_thread = None
        self._dl_state = 'running'
        self._dl_total = getattr(self, '_dl_total_override', len(target))
        self._dl_done_offset = getattr(self, '_dl_done_offset', 0)
        self.prog.setMaximum(self._dl_total)
        self.prog.setValue(self._dl_done_offset)  # 从已完成数开始
        self._dl_total_override = None
        self._dl_done_offset = 0
    ...
    def _pause_or_resume(self):
        if self._dl_state == 'idle': return  # idle 时按钮无效
        if self._dl_state == 'running':
            # 正在下载 → 立刻暂停（同步，不用延迟）
            if self._dl_thread:
                self._dl_thread.stop()
            self._dl_state = 'paused'
            self.stop_btn.setText('▶  继续')
            self.stop_btn.setStyleSheet(
                'background:#14532d;color:#4ade80;border:1px solid #166534;'
                'border-radius:7px;padding:8px 18px;font-size:13px;min-height:32px;')
            self._log('已暂停。点「继续」恢复，点「终止」结束。', 'yellow')
            self.save_btn.setVisible(True); self.discard_btn.setVisible(True)
        elif self._dl_state == 'paused':
            # 已暂停 → 继续
    ...
    def _resume_download(self):
        ...
        if not save_path:
            self._log('无法继续：未找到保存路径。', 'red')
            self._dl_state = 'idle'
            return
        if not self.chapters:
            self._log('无法继续：章节列表已丢失，请重新获取章节后下载。', 'red')
            self._dl_state = 'idle'
            self.stop_btn.setText('⏸  暂停')
            self.stop_btn.setStyleSheet('')
            self.save_btn.setVisible(False); self.discard_btn.setVisible(False)
            self.start_btn.setEnabled(True)
            return
        # stop_btn 重置在 _start_thread 里统一处理
        self.save_btn.setVisible(False); self.discard_btn.setVisible(False)
        # 重新计算剩余章节（跳过已完成的）
        ...
        target = [ch for ch in self.chapters[start:end] if ch[0] not in done]
        if not target:
            self._log('所有章节已下载完成。', 'green'); return
        self._log(f'继续下载，剩余 {len(target)} 章…', 'blue')
        total = self.end_sp.value() - self.start_sp.value() + 1
        self._dl_total_override = total
        self._dl_done_offset    = total - len(target)  # 已完成章节数
        # 确保旧线程信号断开，释放对象，防止继续时崩溃
        if self._dl_thread is not None:
            try:
                self._dl_thread.log.disconnect()
                self._dl_thread.progress.disconnect()
    ...
    def _stop_download_common(self):
        """停止线程，重置按钮状态，返回 (cache, save_path)"""
        ...
        self.save_btn.setVisible(False); self.discard_btn.setVisible(False)
        self.start_btn.setEnabled(True)
        # 安全复制 cache，线程置 None 前先取出
        try:
            cache = dict(self._dl_thread.content_cache) if self._dl_thread else {}
        except: cache = {}
        save_path = getattr(self, '_current_save_path', None)
        if self._dl_thread is not None:
            self._dl_thread.deleteLater()  # Qt安全释放
    ...
    def _save_and_terminate(self):
        """暂停后点「保存」：写文件，清进度，写历史"""
        cache, save_path = self._stop_download_common()
        if not cache or not save_path:
            self._log('终止：无内容可保存。', 'red')
            return
        try:
            with open(save_path, 'a', encoding='utf-8') as f:
                for idx in sorted(cache.keys()):
                    ...
                    if self.settings.get('gen_txt', True):
                        f.write(f"\n{'─'*32}\n{t}\n{'─'*32}\n{c}\n")
            删进度(save_path)
            写历史({'name': self.book_name, 'count': len(cache),
                    'time': datetime.now().strftime('%Y-%m-%d %H:%M'),
                    'path': save_path,
                    'url':  self.url_input.text().strip()})
            self._refresh_history(); self._update_stats()
            self._log(f'已保存 {len(cache)} 章 → {save_path}', 'yellow')
            # 如果开了 epub，也生成
            epub_list = [(v[0],v[1]) for v in
                         [cache[k] for k in sorted(cache.keys())]]
            if epub_list and self.settings.get('gen_epub'):
                et = EpubThread(self.book_name, epub_list, save_path)
                et.done.connect(lambda p: self._log(f'EPUB 已保存：{p}','green'))
                et.fail.connect(lambda e: self._log(f'EPUB 失败：{e}','red'))
                et.start(); self._epub_thread = et
        except Exception as e:
            self._log(f'写入失败：{e}', 'red')

    def _discard_and_terminate(self):
        """暂停后点「舍弃」：清进度，不写文件"""
        _, save_path = self._stop_download_common()
        if save_path:
            删进度(save_path)
        self._log('已舍弃，内容未保存。', 'red')
    ...
    def _setup_url_completer(self):
        """从历史记录里读取网址，设置地址栏下拉补全"""
        from PyQt6.QtWidgets import QCompleter
        from PyQt6.QtCore import QStringListModel
        self._url_completer_model = QStringListModel()
        self._url_completer = QCompleter(self._url_completer_model, self)  # self持有防GC
        ...
        self._url_completer.popup().setStyleSheet(
            'QListView{background:#1e1e1e;color:#d4d4d4;border:1px solid #2e2e2e;'
            'border-radius:6px;font-size:13px;}'
            'QListView::item{padding:6px 12px;}'
            'QListView::item:selected{background:#3b82f6;color:#fff;}')
    ...
    def _on_completer_activated(self, text):
        """补全选中后：填入网址，标记防重弹，自动触发识别"""
        self.url_input._just_activated = True
        self.url_input.setText(text)
        QTimer.singleShot(50, self._auto_fetch)

    def _refresh_url_completer(self):
        """刷新地址栏补全列表（历史网址按下载次数排序）"""
        hist = 读历史()
        # 去重并按出现次数排序
        url_count = {}
        for h in hist:
        ...
        urls = sorted(url_count, key=lambda u: url_count[u], reverse=True)
        if hasattr(self, '_url_completer_model'):
            self._url_completer_model.setStringList(urls)
    ...
    def _result_click(self, item, _):
        ...
        if 0 <= idx < len(self._search_results):
            ...
            # 清空旧书信息，避免误导
            self.book_lbl.setText('正在识别…')
            self.book_lbl.setStyleSheet('color:#555;font-size:13px;font-weight:600;')
            self.chapters = []; self.sel_btn.setEnabled(False)
            self._auto_fetch()
