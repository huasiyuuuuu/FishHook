    ...
    try:
        ...
        for a in soup.find_all('a', href=True):
            ...
            if any(k in t for k in ['目录','返回','书页','章节列表']):
                ...
                try:
                    r2 = safe_get(session, full, adp.headers(full), timeout=10)
        ...
        if m:
            ...
            for guess in [
                ...
                try:
                    r3 = safe_get(session, guess, adp.headers(guess), timeout=8)
                    ...
                    if len(lst3) >= 3:
                        return guess, '自动推断目录页', None
                except: continue
        return url, '未知页面（直接尝试）', None
    except Exception as e:
...
class ChapterDialog(QDialog):
    ...
    def _select_all(self):
        ...
        for i in self._items: i.setCheckState(Qt.CheckState.Checked)
...
class MainWindow(QMainWindow):
    ...
    def _show_guide(self):
        FIRST_RUN.touch()
        QMessageBox.information(self, '欢迎使用 Hook',
            '三步开始下载：\n\n'
            '① 在下载页粘贴小说目录页网址（或章节页网址也可以）\n'
            '② 点「识别并获取」获取章节列表\n'
            '③ 点「开始下载」\n\n'
            '搜索页：选择已访问过的网站，输入书名搜索\n'
            '历史页：查看下载记录，右键可继续下载或删除')
    ...
    def _start_with(self, target):
        ...
        if not save_dir:
            QMessageBox.warning(self, '提示', '请设置保存位置'); return
        # 磁盘空间检查
        try:
    ...
    def _hist_context_menu(self, pos):
        ...
        if action == a_open:
            ...
            # 跳到下载页，填入最后的网址（从历史里没存网址，跳转提示用户）
            QMessageBox.information(self,'提示',
                f'请在下载页重新输入《{name}》的目录页网址并获取章节。')
            self.tabs.setCurrentIndex(0)
    ...
    def _reset(self):
        if QMessageBox.question(self,'确认','重置所有设置？') == QMessageBox.StandardButton.Yes:
            写设置(DEFAULTS)
            QMessageBox.information(self,'完成','已重置，重启生效。')
    ...
    def _on_check_result(self, ver, newer):
        ...
        if newer:
            ...
            if QMessageBox.question(self,'发现更新',msg) == QMessageBox.StandardButton.Yes:
                self._do_update()
        else:
    ...
    def _on_update_done(self, ok, err):
        self.upd_prog.hide()
        if ok:
            QMessageBox.information(self,'更新完成','新版本已下载，点确定后自动重启。')
            restart_app()
        else:
