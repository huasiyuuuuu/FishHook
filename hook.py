...
class MsgBox(QDialog):
    ...
    def __init__(self, parent, title, text, mode=0):
        ...
        """)
        L = QVBoxLayout(self)
        L.setContentsMargins(24,20,24,20); L.setSpacing(16)
...
class MainWindow(QMainWindow):
    ...
    def _on_fetch_fail(self, err):
        ...
        if 'SSL' in err or 'EOF' in err:
            self._log('提示：SSL连接错误，该网站可能需要梯子，请重试', 'yellow')
        elif 'timeout' in err.lower():
            self._log('提示：连接超时，请检查网络或稍后重试', 'yellow')
        elif 'connection' in err.lower():
            self._log('提示：无法连接，请确认网址正确且网站可访问', 'yellow')
        else:
            self._log('提示：请确认粘贴的是小说目录页（含章节列表的页面）', 'yellow')
        self.fetch_btn.setEnabled(True)
    ...
    def _hist_context_menu(self, pos):
        ...
        if action == a_open:
            ...
            MsgBox.info(self, '提示', '请在下载页重新输入《{name}》的目录页网址并获取章节。')
            self.tabs.setCurrentIndex(0)
